...
| **Issuer in requests** | **Issuance API paths stay unchanged.** `issuerId` is passed in the request (body / query param). Certify filters by issuer and runs the **same existing flow**. |
...
4. GET /.well-known/openid-credential-issuer?issuerId=farmer
...
| **Well-known URLs** | Same paths — optional `?issuerId=` query param for per-issuer metadata |
...
GET  /.well-known/openid-credential-issuer?issuerId=...  → WellKnownController (CHANGED: filter by issuer)
GET  /.well-known/did.json?issuerId=...                → WellKnownController (CHANGED: filter by issuer)
...
### 4.3 CHANGED — Discovery (well-known)

**Paths stay the same.** Add optional `issuerId` query parameter.

```
Before:  GET /.well-known/openid-credential-issuer           → all configs, one credential_issuer
After:   GET /.well-known/openid-credential-issuer?issuerId=farmer  → configs for farmer only

         GET /.well-known/openid-credential-issuer           → default issuer (when issuerId omitted)
         GET /.well-known/did.json?issuerId=farmer          → DID doc for farmer only
```

| Response field | Source after change |
|----------------|---------------------|
| `credential_issuer` | `issuer.credential_issuer_url` (or `mosip.certify.domain.url` for `default`) |
...
DID document: `GET /.well-known/did.json?issuerId=farmer` — built from issuer's `did_url` and that issuer's credential configs only.

**Wallet / Mimoto:** `wellknown_endpoint` can append query param, e.g.  
`http://certify/.well-known/openid-credential-issuer?issuerId=farmer`
...
### 5.2 CHANGED — Discovery (same paths, optional `issuerId` query param)

| Method | Path | Change |
|--------|------|--------|
| GET | `/v1/certify/.well-known/openid-credential-issuer?issuerId={issuerId}` | Filter metadata by issuer |
| GET | `/v1/certify/.well-known/did.json?issuerId={issuerId}` | Per-issuer DID document |
| GET | `/v1/certify/.well-known/jwks.json?issuerId={issuerId}` | Per-issuer JWKS (if applicable) |

When `issuerId` is omitted → `default` issuer (backward compatible).
...
```json
{
  "issuerId": "farmer",
  "status": "ACTIVE",
  "credentialIssuerUrl": "https://certify.example.com/v1/certify",
  "identifier": "https://certify.example.com/v1/certify",
  "didUrl": "did:web:certify.example.com:issuers:farmer",
  "keyManagerAppId": "CERTIFY_ISSUER_FARMER_ED25519",
  "keyManagerRefId": "ED25519_SIGN",
  "wellKnownEndpoints": {
    "openidCredentialIssuer": "https://certify.example.com/v1/certify/.well-known/openid-credential-issuer?issuerId=farmer",
    "didJson": "https://certify.example.com/v1/certify/.well-known/did.json?issuerId=farmer"
  }
}
```
...
| 6 | Update Mimoto `wellknown_endpoint` with `?issuerId=` per issuer | Ops |
...
| `WellKnownController.java` | Add optional `@RequestParam issuerId`; pass to metadata/DID services |
...
| `mimoto-issuers-config.json` | `wellknown_endpoint` with `?issuerId=` (docker reference) |
...
- [ ] Verify `GET /.well-known/openid-credential-issuer?issuerId={id}`
- [ ] Verify `GET /.well-known/did.json?issuerId={id}`
- [ ] Update wallet config: `wellknown_endpoint` + credential request includes `issuerId`
...
- [ ] DB: `issuer` table + `credential_config.issuer_id` + migration
- [ ] Issuer onboarding API (`POST /issuers`) with Keymanager integration
- [ ] Issuer CRUD (GET, PUT, DELETE)
- [ ] `IssuerResolver` + `IssuerContext` (from request body / query param — **not** path filter)
- [ ] `CredentialRequest.issuerId` + well-known `?issuerId=` query param
- [ ] Refactor metadata, DID, issuance to filter by issuer — **same issuance URLs**
- [ ] `credential-configurations` requires `issuerId`
- [ ] Slim `AppConfig.initKeys` (platform keys only)
- [ ] `default` issuer bootstrap for backward compatibility
- [ ] Unit + integration tests (including cross-issuer isolation)
- [ ] OpenAPI + docker injistack reference update
...
| 2 | `issuerId` in well-known: query param vs header? | Query param `?issuerId=` (consistent, cacheable URLs) |
